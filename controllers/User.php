...
class User extends CI_Controller
{
  ...
  {
    $data = array(
              'list_data' => $this->M_user->select('tb_barang_masuk'),
  ...
  {
     $data = array(
              'list_data' => $this->M_user->select('tb_barang_persetujuan'),
              'avatar'    => $this->M_user->get_data_gambar('tb_upload_gambar_user',$this->session->userdata('name'))
            );
    $this->load->view('user/tabel/tabel_barangpersetujuan',$data);
  }
  ...
  public function proses_databarang_masuk_insert()
  {
    ...
    if($this->form_validation->run() == TRUE)
    {
      ...
      redirect(base_url('user/form_barangmasuk'));
    }else {
      $data['list_satuan'] = $this->M_user->select('tb_satuan');
      $this->load->view('user/form_barangmasuk/form_insert',$data);
    }
  ...
  public function proses_persetujuan_data()
  {
    ...
    if($this->form_validation->run() === TRUE)
    {
      ...
      $data = array(
        ...
        $this->M_user->insert('tb_barang_persetujuan',$data);
        $where = array('id_transaksi' => $id_transaksi);
        $this->M_user->delete('tb_barang_masuk',$where);
